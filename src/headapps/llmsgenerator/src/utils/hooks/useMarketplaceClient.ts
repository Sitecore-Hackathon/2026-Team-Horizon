// src/utils/hooks/useMarketplaceClient.ts
...
async function getMarketplaceClient() {
  ...
  const config = {
    ...
    modules: [XMC], // Initialize XMC package for XM Cloud APIs
  };
...
export function useMarketplaceClient(options: UseMarketplaceClientOptions = {}) {
  ...
  const initializeClient = useCallback(async (attempt = 1): Promise<void> => {
    ...
    try {
      console.log("Initializing Marketplace client with XMC module...");
      ...
      if (attempt < opts.retryAttempts) {
        console.log(`Retrying... Attempt ${attempt + 1} of ${opts.retryAttempts}`);
